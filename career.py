...
data = {
    ...
    "career": ["AI Engineer", "Software Developer", "Data Scientist", "AI Engineer",
...
def recommend_career(cgpa, python, ml, communication):
    prediction = model.predict([[cgpa, python, ml, communication]])[0]
    probs = model.predict_proba([[cgpa, python, ml, communication]])[0]
...
with tab1:
    ...
    if st.button("🔮 Predict Career"):
        python_val = 1 if python == "Yes" else 0
        ml_val = 1 if ml_skill == "Yes" else 0
        comm_val = 1 if comm == "Yes" else 0
        career, skills, prob_df = recommend_career(cgpa, python_val, ml_val, comm_val)
